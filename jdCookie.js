...
!(async () => {
    ...
    if (tokenResp.data.token) {
        ...
        for (let jdcookie of JD_COOKIES) {
            if (jdcookie.value.indexOf("wskey") !== -1) {
                jdcookie.value = jdcookie.value.replace(/wskey=.*;/g,'')
            }
        }
